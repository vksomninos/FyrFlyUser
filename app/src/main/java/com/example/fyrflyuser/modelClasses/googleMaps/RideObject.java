package com.example.fyrflyuser.modelClasses.googleMaps;
...
public class RideObject implements Cloneable{
    ...
    private String  requestService = "type_1", car = "--", cancelledReason, customerName;
    ...
    Boolean ended = false, customerPaid = false, driverPaid = false, cancelled = false  , rideRequest = false;
    ...
    public void parseData(DataSnapshot dataSnapshot){
        ...
        if(dataSnapshot.child("rideRequest").getValue() != null){
            rideRequest = Boolean.parseBoolean(dataSnapshot.child("rideRequest").getValue().toString());
        }
    ...
    public void postRideInfo(){
        ...
        String customerId = CommonUtils.getUserId();
        HashMap map = new HashMap();
        map.put("service", requestService);
        map.put("state", 0);
        map.put("customerId", customerId);
        map.put("ended", false);
        map.put("rideRequest",rideRequest);
        map.put("customerName",customerName);

        map.put("calculated_duration", duration);
        map.put("calculated_distance", distance);
        map.put("calculated_Price", CommonUtils.rideCostEstimate(distance, duration));
